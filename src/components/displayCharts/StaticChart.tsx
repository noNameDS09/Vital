...
import { patients } from '../../app/profile/data';
...
const PatientInfoCard = ({ patient, onClick, isActive }) => (
  <div
    className={`flex-shrink-0 w-64 p-4 rounded-2xl bg-[var(--card)] border cursor-pointer transition-colors duration-200 ${
      isActive ? 'border-[var(--primary)] shadow-lg' : 'border-[var(--border)]'
    }`}
  ...
  >
    <div className="flex items-center space-x-4">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--muted-foreground)]">ID: {patient.patientId}</p>
      </div>
    </div>
  </div>
);
...
const App = () => {
  ...
  return (
    <div className="bg-[var(--background)] min-h-screen p-8 text-[var(--foreground)] font-sans">
